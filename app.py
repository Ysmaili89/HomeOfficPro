...
if IS_WINDOWS:
    ...
    DATABASE_PATH = os.path.join(BASE_DIR, 'filtro_amazon.db')
else:
    # Linux/Mac - PythonAnywhere
    DATABASE_PATH = '/home/Ysmailin89/filtro_amazon.db'
...
# ========== MIDDLEWARE PARA ERRORES DE SESIÓN ==========
@app.teardown_appcontext
def shutdown_session(exception=None):
    """Cierra la sesión de base de datos después de cada petición"""
    if exception:
        try:
            db.session.rollback()
        except Exception as e:
            logger.error(f"Error en rollback de sesión: {e}")
    try:
        db.session.remove()
    except Exception as e:
        logger.error(f"Error removiendo sesión: {e}")
...
def get_categorias_lista():
    """Obtiene solo los IDs de categorías (para compatibilidad)"""
    try:
        return [c.categoria_id for c in Categoria.query.all()]
    except Exception as e:
        logger.error(f"Error obteniendo lista de categorías: {e}")
        return []
...
def panel_dashboard():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_dashboard: {e}")
...
def panel_productos():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_productos: {e}")
...
def panel_producto_nuevo():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_producto_nuevo: {e}")
...
def panel_producto_editar(producto_id):
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_producto_editar: {e}")
...
def panel_producto_eliminar(producto_id):
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_producto_eliminar: {e}")
        flash('Error al eliminar el producto', 'error')
...
def panel_producto_duplicar(producto_id):
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_producto_duplicar: {e}")
        flash('Error al duplicar el producto', 'error')
...
def panel_categorias():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_categorias: {e}")
...
def panel_categoria_nueva():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_categoria_nueva: {e}")
        flash('Error al crear la categoría', 'error')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if not nuevo_nombre:
                ...
                return redirect(url_for('panel_categoria_editar', categoria_id=categoria.id))
            
            try:
                categoria.nombre = nuevo_nombre
                db.session.commit()
                logger.info(f"Categoría actualizada: {categoria.categoria_id}")
        ...
        db.session.rollback()
        logger.error(f"Error en panel_categoria_editar: {e}")
...
def panel_categoria_eliminar(categoria_id):
    try:
        ...
        productos_en_categoria = Producto.query.filter_by(categoria=categoria.categoria_id).count()
        ...
        db.session.rollback()
        logger.error(f"Error en panel_categoria_eliminar: {e}")
        flash('Error al eliminar la categoría', 'error')
...
def panel_usuario_nuevo():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_usuario_nuevo: {e}")
        flash('Error al crear el usuario', 'error')
...
def panel_usuario_editar():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_usuario_editar: {e}")
...
def panel_usuario_eliminar(username):
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_usuario_eliminar: {e}")
        flash('Error al eliminar el usuario', 'error')
...
def panel_configuracion():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_configuracion: {e}")
...
def panel_config_afiliado():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_config_afiliado: {e}")
        flash('Error al guardar la configuración', 'error')
...
def panel_config_seo():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_config_seo: {e}")
        flash('Error al guardar la configuración', 'error')
...
def panel_config_apariencia():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en panel_config_apariencia: {e}")
        flash('Error al guardar la configuración', 'error')
...
def panel_notificaciones():
    try:
        ...
        logger.error(f"Error en panel_notificaciones: {e}")
...
def nosotros():
    try:
        ...
        return render_template('nosotros.html', categorias=get_categorias())
    except Exception as e:
        logger.error(f"Error en nosotros: {e}")
        return render_template('nosotros.html', categorias=[])

@app.route('/contacto')
def contacto():
    try:
        actualizar_estadisticas('contacto')
        return render_template('contacto.html', categorias=get_categorias())
    except Exception as e:
        logger.error(f"Error en contacto: {e}")
        return render_template('contacto.html', categorias=[])
...
def enviar_contacto():
    try:
        ...
        logger.error(f"Error en enviar_contacto: {e}")
...
def enviar_contacto_filtro():
    try:
        ...
        logger.error(f"Error en enviar_contacto_filtro: {e}")
        flash('Error al enviar el mensaje', 'error')
    return redirect(url_for('contacto'))
...
def faq():
    try:
        ...
        return render_template('faq.html', categorias=get_categorias())
    except Exception as e:
        logger.error(f"Error en faq: {e}")
        return render_template('faq.html', categorias=[])

@app.route('/mapa-web')
def mapaweb():
    try:
        actualizar_estadisticas('mapa-web')
        return render_template('mapaweb.html', categorias=get_categorias())
    except Exception as e:
        logger.error(f"Error en mapaweb: {e}")
        return render_template('mapaweb.html', categorias=[])

@app.route('/cookies')
def cookies():
    try:
        actualizar_estadisticas('cookies')
        return render_template('cookies.html', categorias=get_categorias())
    except Exception as e:
        logger.error(f"Error en cookies: {e}")
        return render_template('cookies.html', categorias=[])

@app.route('/terminos')
def terminos():
    try:
        actualizar_estadisticas('terminos')
        return render_template('terminos.html', categorias=get_categorias())
    except Exception as e:
        logger.error(f"Error en terminos: {e}")
        return render_template('terminos.html', categorias=[])

@app.route('/privacidad')
def privacidad():
    try:
        actualizar_estadisticas('privacidad')
        return render_template('privacidad.html', categorias=get_categorias())
    except Exception as e:
        logger.error(f"Error en privacidad: {e}")
        return render_template('privacidad.html', categorias=[])

@app.route('/legal')
def legal():
    try:
        actualizar_estadisticas('legal')
        return render_template('legal.html', categorias=get_categorias())
    except Exception as e:
        logger.error(f"Error en legal: {e}")
        return render_template('legal.html', categorias=[])
...
def legal_privacidad():
    try:
        ...
        logger.error(f"Error en legal_privacidad: {e}")
        return render_template('privacidad.html', categorias=[])
...
def legal_cookies():
    try:
        ...
        logger.error(f"Error en legal_cookies: {e}")
        return render_template('cookies.html', categorias=[])
...
def legal_aviso():
    try:
        ...
        logger.error(f"Error en legal_aviso: {e}")
        return render_template('legal.html', categorias=[])
...
def legal_afiliados():
    try:
        ...
        logger.error(f"Error en legal_afiliados: {e}")
        return render_template('legal.html', categorias=[])
...
def categorias_page():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en categorias_page: {e}")
        return render_template('categorias.html',
                             categorias=[],
                             productos=[])
...
def recomendados():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error en recomendados: {e}")
        return redirect(url_for('index'))
...
def api_producto(producto_id):
    try:
        ...
        logger.error(f"Error en api_producto: {e}")
...
def api_categorias():
    try:
        ...
        logger.error(f"Error en api_categorias: {e}")
        return jsonify([])
...
def api_estadisticas():
    try:
        ...
        logger.error(f"Error en api_estadisticas: {e}")
        return jsonify({'error': 'Error interno'}), 500
...
def api_buscar():
    try:
        ...
        logger.error(f"Error en api_buscar: {e}")
        return jsonify([])
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not db_exists and 'sqlite' in app.config['SQLALCHEMY_DATABASE_URI']:
            ...
            # Verificar que la base de datos exista y tenga tablas
            try:
